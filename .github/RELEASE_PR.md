...
- [ ] I published the latest version on TestPyPI and checked that the following command work:
       ```pip install ltlf2dfa==<version-number> --index-url https://testpypi.org/simple --force --no-cache-dir --no-deps```
- [ ] After merging the PR, I'll publish the build also on PyPI. Then, I'll make sure the following
      command will work:
      ```pip install ltlf2dfa==<version_number> --force --no-cache-dir --no-deps```  
